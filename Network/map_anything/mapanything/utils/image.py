...
def load_images_v2(
    ...
    """Preprocess in-memory image tensors using the same pipeline as ``load_images``.

    Args:
        tensor_batches (Iterable[torch.Tensor]): Iterable of tensors shaped (B, 3, H, W)
            with float values in [0, 1]. Each tensor may have a different batch size.
        resize_mode (str): Same as :func:`load_images`.
        size (int or tuple, optional): Same as :func:`load_images`.
        norm_type (str): Same as :func:`load_images`.
        patch_size (int): Same as :func:`load_images`.
        verbose (bool): Same as :func:`load_images`.
        resolution_set (int): Same as :func:`load_images`.
        stride (int): Load every ``stride``-th image from the flattened tensor batches.

    Returns:
        list: List of dictionaries formatted identically to :func:`load_images`.
    """
    ...
    if not isinstance(tensor_batches, (list, tuple)):
        raise TypeError(
            "tensor_batches must be a list or tuple of torch.Tensor batches shaped (B, 3, H, W)"
        )

    to_pil = tvf.ToPILImage()
    aspect_ratios = []
    processed_inputs = []
    global_index = 0
    ...
    for batch_idx, batch in enumerate(tensor_batches):
        if not isinstance(batch, torch.Tensor):
            raise TypeError(
                f"Expected torch.Tensor at tensor_batches[{batch_idx}], got {type(batch)}"
            )
        if batch.ndim != 4 or batch.shape[1] != 3:
            raise ValueError(
                f"Expected tensor shaped (B, 3, H, W) at tensor_batches[{batch_idx}], got {batch.shape}"
            )

        batch_cpu = batch.detach().cpu().float()
        if batch_cpu.numel() == 0:
            continue

        if batch_cpu.min().item() < -1e-4 or batch_cpu.max().item() > 1.0 + 1e-4:
            raise ValueError(
                f"tensor_batches[{batch_idx}] contains values outside [0, 1] after casting to float"
            )

        batch_cpu = batch_cpu.clamp(0.0, 1.0)

        for inner_idx in range(batch_cpu.shape[0]):
            if global_index % stride != 0:
                global_index += 1
                continue

            tensor_img = batch_cpu[inner_idx]
    ...
    if not processed_inputs:
        raise ValueError("No valid images found in tensor_batches")
    ...
    imgs = []
    for idx, entry in enumerate(processed_inputs):
        pil_image = entry["pil_image"]
        original_width, original_height = entry["original_size"]

        resized_image = crop_resize_if_necessary(pil_image, resolution=target_size)[0]
        new_width, new_height = resized_image.size

        if verbose:
            print(
                " - Adding tensor"
                f"[{entry['batch_index']}][{entry['tensor_index']}] "
                f"with resolution {original_width}x{original_height} --> {new_width}x{new_height}"
            )

        imgs.append(
            dict(
                img=ImgNorm(resized_image)[None],
    ...
    assert imgs, "No images found in tensor_batches"
    if verbose:
        print(f" (Processed {len(imgs)} images from tensors)")
