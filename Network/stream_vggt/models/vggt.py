...
from ..models.aggregator import StreamAggregator
...
class StreamVGGT(nn.Module, StreamLike, PyTorchModelHubMixin):
    def __init__(self, img_size=518, patch_size=14, embed_dim=1024):
        super().__init__()
        self.stages = (4, 11, 17, 23)

        self.aggregator = StreamAggregator(
            img_size=img_size, patch_size=patch_size, embed_dim=embed_dim, output_stages=self.stages
        )
        ...
        self.point_head = DPTHead(
            ...
            intermediate_layer_idx=[_ for _ in range(len(self.stages))]
        )
    ...
    def succ(self, new_image: torch.Tensor) -> Any:
        ...
        aggregated_tokens_list, patch_start_idx = self.aggregator.succ(new_image)
        ...
        with torch.autocast('cuda', enabled=False):
            ...
            if self.depth_head is not None:
                depth, depth_conf = self.depth_head(
                    aggregated_tokens_list, images=new_image, patch_start_idx=patch_start_idx
                )
                predictions["depth"] = depth
                predictions["depth_conf"] = depth_conf
